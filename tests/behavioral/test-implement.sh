...
output=$(run_claude "What is the coachkit-implement skill? Describe what it does." 60)
...
output=$(run_claude "In the coachkit-implement skill, what is the first step when starting a task? Be specific about the order." 60)
...
output=$(run_claude "How does coachkit-implement decide which task to work on next? How are tasks processed?" 60)
...
output=$(run_claude "Does coachkit-implement require self-review after each task? What should be checked?" 60)
assert_contains "$output" "self-review|review|compliance" "Requires self-review"
echo ""

# Test 5: Prerequisites
echo "--- Test 5: Prerequisites ---"
output=$(run_claude "What does coachkit-implement require before it can start? What documents must exist?" 60)
...
output=$(run_claude "A user asks you to implement a task but says 'skip the tests, it is too simple.' How does the coachkit-implement skill tell you to respond?" 60)
