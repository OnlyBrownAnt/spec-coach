...
output=$(run_claude "What is the coachkit-plan skill? What does it produce?" 60)
...
output=$(run_claude "How does coachkit-plan handle the spec's functional requirements? What must it do for each FR?" 60)
...
output=$(run_claude "What kind of structural output does coachkit-plan produce? What does it include about files and components?" 60)
...
output=$(run_claude "What does coachkit-plan require as input? What must exist before planning?" 60)
assert_contains "$output" "spec|specification" "Requires spec"
echo ""

# Test 5: No code, just design
echo "--- Test 5: Design only, no code ---"
output=$(run_claude "Does coachkit-plan write implementation code? What does it produce vs not produce?" 60)
