...
- [ ] **T003** `[US1,US2]` Edit `skills/constitution.md` (source): add a **"Constitution state"** preamble — run `verify-constitution-sync.sh` to get status, then branch: `AUTHORED` → **amend path** (anchor to the existing principle set; never rewrite a settled principle unless it is the explicit amendment target; full rewrite only via explicit `--reset`); `TEMPLATE`/`ABSENT` → **seeded cold-start path** (read `package.json` name+deps, the primary source/`skills` dir, `README.md`, existing `specs/`; PROPOSE candidate principles + the two flexible sections for human ratification; write nothing until approved). Reframe "Your Role" to state the charter-as-IP principle (CLAUDE.md-tier; amend never overwrite). **Regenerate** the installed `.claude/skills/spec-constitution/SKILL.md` from the edited source (run the install path so frontmatter is added and the tracked installed copy stays in sync). Add **content-assertion tests** to `tests/units/constitution-charter.test.ts`: read `skills/constitution.md`, assert the amend-guard, status-branch, and seeded-cold-start guidance substrings are present.
