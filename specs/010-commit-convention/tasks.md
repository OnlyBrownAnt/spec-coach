...
- [ ] **T003** `[US1]` Make `uninstall` status-aware for the convention in `src/commands/uninstall.ts` (FR-001 charter-as-IP extension): add `isAuthoredConvention(p)` mirroring `isAuthoredConstitution` (signature-token test for `[PROJECT_NAME]`/`[ALLOWED_TYPES]`/`[SCOPE_FORMAT]`); add a step 2c after the memory handling — preserve an AUTHORED `.spec/convention.md` on plain uninstall; remove it on purge or when still TEMPLATE. **RED-first** tests in `tests/units/commit-convention.test.ts`: AUTHORED preserved on plain uninstall; TEMPLATE removed on plain uninstall; both removed on `--force`.
